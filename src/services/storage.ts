...
class StorageService {
  ...
  public initialize(): void {
    ...
    const CURRENT_VERSION = 'tin_hoc_v3_server';
  ...
  // Fetch full data from server & update local cache
  public async syncFromServer(): Promise<boolean> {
    ...
    try {
      ...
      // Check if version changed or counts changed
      ...
      const hasChanged =
        ...
        data.tasks.length !== localTasks.length ||
        data.submissions.length !== localSubs.length ||
        data.games.length !== localGames.length;
      ...
      if (hasChanged) {
        ...
        if (Array.isArray(data.tasks)) this.setItem(STORAGE_KEYS.TASKS, data.tasks);
        if (Array.isArray(data.submissions)) this.setItem(STORAGE_KEYS.SUBMISSIONS, data.submissions);
